...
public class AuthenticateController {
	...
	public ResponseEntity<Boolean> SignUpUser(@RequestBody User user)
	{
		...
		return restTemplate.postForEntity("http://localhost:8888/identity/signup",user,Boolean.class); 
	}
	...
	public ResponseEntity<String> checkData(@RequestBody Authrequest authrequest) throws Exception
	{
		...
		return restTemplate.postForEntity("http://localhost:8888/identity/authenticate", authrequest,String.class);
	}
	
	@GetMapping("/findByEmail/{emailId}")
	public ResponseEntity<Boolean> findbyEmail(@PathVariable String emailId)
	{
		return restTemplate.getForEntity("http://localhost:8888/identity/findByEmail/"+emailId, Boolean.class);
	}
